...
def load_grammar(path_to_grammar):
	...
	with open(path_to_grammar, 'r+', encoding="utf8") as f:
		return (json.load(f))
...
def get_rule(grammar, key):
	...
	if key not in grammar:
		return f"(({key}))"
	elif isinstance(grammar[key], str):
		return grammar[key]
	else:
...
def apply_modifiers(modifier_names, text):
	...
	for key in modifier_names:
		my_fxn = modifier_dictionary[key]
		text = my_fxn(text)
	return text
...
def expand_rule_section(grammar, section_text, index):
	...
	*** A helper function for expand_text *** 
	...
	if index % 2 == 0:
		...
		split_text = section_text.split(".")
		key = split_text[0]
		modifier_names = split_text[1::]
		rule = get_rule(grammar, key)
		return apply_modifiers(modifier_names, expand_rule(grammar, rule))
...
def expand_rule(grammar, rule):
	...
	* Use "".join(some_list) to merge the list back into a single string
	...
	result_rule= [expand_rule_section(grammar, ele, i) for i, ele in enumerate(split_rule, start=0)]
...
def respond_to_input(grammar, user_input):
	...
	for key in grammar:
		if key in user_input:
			return expand_rule(grammar, f"#{key}#")
...
my_grammar = {
	"food": ["nuggets made of coal", "smoothies", "fried Tyrannosaurus wings"],
	"topic": ["poetry", "stories", "#food# recipes", "#food# philosophy", "#food# shows"],
	"game": ["mario", "mancala", "#topic# related games", "#food# throwing games"],
	"movie": ["Jurassic Park", "Star Wars", "Cars", "a #food# related film", "a movie about #game#", "a #topic# related movie"],
	"idea": "#movie.a# buff",
	"origin": "We were having a discussion about #topic# when our #food# arrived. We brought it to the living room and put on #movie#. I am #idea#, so it was cool!"
}
...
if __name__ == '__main__':
	...
	# #----------------------------
	# # Task 5
	print(expand_rule_section(grammar, "animal", 0))
	print(expand_rule_section(grammar, "animal.a.capitalize", 1))
	...
	for i in range(0, 10):
		finished = expand_rule(grammar, modifier_rule2)
		print(finished)
