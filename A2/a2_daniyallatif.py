...
def is_name_match(name, query):
	# I just made this function so I could invoke is_breed_match again for names without it being confusing.
	return is_breed_match(name, query)
...
def get_breed_data_by_name(breeds, query_name):
	...
	for breed in breeds:
		if is_breed_match(breed[INDEX_BREED_NAME], query_name):
			return breed
	return None
...
def get_breed_data_for_dog(breeds, dog_data):
	...
	return get_breed_data_by_name(breeds, dog_data[INDEX_DOG_BREED])
...
def has_temperament(breed_data, adjective):
	...
	if not breed_data:
		...
		return adjective in breed_data[INDEX_BREED_TEMPERAMENT]
...
def list_all_zip_codes(dogs):
	...
	zips = []
	for dog in dogs:
		if dog[INDEX_DOG_ZIPCODE] not in zips:
			zips.append(dog[INDEX_DOG_ZIPCODE])
	return sorted(zips)
...
def list_breeds(dogs):
	...
	breed_names = []
	for dog in dogs:
		if dog[INDEX_DOG_BREED] not in breed_names:
			breed_names.append(dog[INDEX_DOG_BREED])
	return sorted(breed_names)
...
def search_dogs(dogs, breeds, query_zip_code=None,query_positive_traits=None,query_negative_traits=None,query_name=None,query_breed=None):
	...
	Returns a sorted list of all dogs in the dogs set that pass the provided filters
		...
		Parameters:
			zip_code (str): if provided, the zipcode the dog must have
			positive_traits (list): the dog's breed temperament must contain ALL of these
			negative_traits (list): the dog's breed temperament must contain NONE of these
			name (str): if provided, the dog's name must match this (case insensitive)
			breed (str): if provided, the dog's name must match this (using is_breed_match)
		Returns:
	...
	def dog_is_match(dog):
		...
		if query_zip_code != None:
			if query_zip_code != dog[INDEX_DOG_ZIPCODE]:
				return False
		if query_name != None:
			if not is_name_match(dog[INDEX_DOG_NAME], query_name):
				return False
		if query_breed != None:
			if not is_breed_match(dog[INDEX_DOG_BREED], query_breed):
				return False
		breed_data = get_breed_data_for_dog(breeds, dog)
		if breed_data == None:
			return False
		if query_positive_traits != None:
			for adjective in query_positive_traits:
				if not has_temperament(breed_data, adjective):
					return False
		if query_negative_traits != None:
			for adjective in query_negative_traits:
				if has_temperament(breed_data, adjective):
					return False
		return True	
...
if __name__ == "__main__":
	...
	# # # Test TASK 10
	print([dog_to_string(dog) for dog in search_dogs(test_dogs,  breeds, query_zip_code="10023")])

	
	print([dog_to_string(dog) for dog in search_dogs(test_dogs,  breeds, query_positive_traits=["Devoted", "Intelligent", "Gentle", "Reserved"])])
	print([dog_to_string(dog) for dog in search_dogs(test_dogs,  breeds, query_positive_traits=["Devoted", "Gentle"], query_negative_traits=["Intelligent"])])
	...
	# # Finds two unknown and two Labradors
	...
	#print([dog_to_string(dog) for dog in baby_dogs])
