...
def get_title(soup):
	...
	return text[0:text.find(" - Wikipedia")]
...
def get_last_edit(soup):
	...
	* Finally, we don't want the whole text, 
	...
	text = soup.find(id="footer-info-lastmod").text
	return text[text.find(start_text)+4:text.find(end_text)]
...
def get_wordcount(soup):
	...
	To get all the text of a whole page, use "soup.text"
...
def get_all_wiki_link_ids(soup):
	...
	* don't have ":" or "#"
	...
	prefix = "/wiki/"
	all_links_with_prefix = [x for x in get_all_links(soup) if x.startswith(prefix)]
	all_links_with_prefix_and_no_colon = [x for x in all_links_with_prefix if ":" not in x]
	return [x[6::] for x in all_links_with_prefix_and_no_colon if "#" not in x]
...
def get_shortest_page(page_directory):
	...
	for page in page_directory:
		if page_directory[page].wordcount < winning_count:
			winning_count = page_directory[page].wordcount
			winner = page_directory[page]
	return winner
...
def get_oldest_page(page_directory):
	...
	winner = None
	for page in page_directory:
		if datetime.strptime(page_directory[page].last_edit,'%d %B %Y') < winning_timestamp:
			winning_timestamp =  datetime.strptime(page_directory[page].last_edit,'%d %B %Y')
			winner = page_directory[page]
	return winner	
...
class WikiPage:
	def __init__(self, page_directory, page_id):
		...
		loaded_html = load_url_politely(self.url)
		self.soup = BeautifulSoup(loaded_html, 'html.parser')
		...
		# Will print all the HTML
		# print(self.soup)
		# # Will only print the plaintext
		# print(self.soup.text)

		# Uncomment to see a summary of this page after initializing it
		print(f"Created page '{self.page_id}'")
		self.print_summary()
	...
	def load_links(self, recursion_count=0, links_per_article=5):
		...
		# Create a new (shorter) list of the first "links_per_article" wiki ids 
		#	from self.wiki_links:
		# 	(ie, ['Felidae', 'Cat_(disambiguation)', 'Cats_(disambiguation)', 
		#   'Conservation_status', 'Taxonomy_(biology)'] in the Cats article)
		# For each wiki id in the short list, 
		...
		#			* Call load_links on the new page, (like we do in main) 
		#				but decrease the recursion_count by 1!!!

		print(self.wiki_links[0:links_per_article])
		short_wiki_id_list = self.wiki_links[0:links_per_article]
		for wiki_id in short_wiki_id_list:
			if wiki_id not in self.page_directory:
				self.page_directory[wiki_id] = WikiPage(self.page_directory, wiki_id)
				if recursion_count > 0:
					self.page_directory[wiki_id].load_links(recursion_count - 1, links_per_article)
	...
	def add_to_tree(self, tree):
		...
		# Pages have a lot of links, only show the first 20
...
if __name__ == "__main__":
	...
	# test_url = "https://en.wikipedia.org/wiki/Northwestern_University"
	...
	# You can also 
	test_html = load_url_politely(test_url)
	...
	expected_cat_count = 15429
	expected_cat_edit = "11 February 2022"
	...
	if test_edit != expected_cat_edit:
		print(f"*** POSSIBLE ERROR: Don't worry if this isn't exact (articles get edited), but your edit date is {test_edit} and we expected something close to {expected_cat_edit}. Make sure you get about the right number and the right format")
	...
	assert "Conservation_status" in page_directory, "If it loaded the first 5 links from Cat, 'Conservation_status' should be in the directory (may change if Wikipedia is edited)"
	...
	print("Total pages loaded", len(page_directory.keys()))
